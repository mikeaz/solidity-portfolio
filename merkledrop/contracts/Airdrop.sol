...
//you are allowed one additional contract import
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

/// @title Airdrop
/// @author Michael Azorin, Melvillian
...
contract Airdrop is Ownable {
    ...
    function signatureClaim(bytes memory signature, address _to, uint256 _amount) external {
        ...
        bytes32 digest = toTypedDataHash(msg.sender, _amount);
        ...
        //transform signature into v, r, s:
        bytes32 r;
        bytes32 s;
        uint8 v;
        assembly {
        ...
        require(signatory == signer, "invalid signature");

        alreadyClaimed[msg.sender] = true;
        macroToken.safeTransfer(_to, _amount);
    ...
    function merkleClaim(bytes32[] calldata _proof, address _to, uint256 _amount) external {
        ...
        bytes32 leaf = toLeafFormat(msg.sender, _amount);

        require(processProof(_proof, leaf) == merkleRoot, "invalid merkle");
        alreadyClaimed[msg.sender] = true;
        macroToken.safeTransfer(_to, _amount);
        emit AirdropClaimed(msg.sender, _amount);
    }
    ...
    function hashPair(bytes32 a, bytes32 b) private pure returns (bytes32 pairHash) {
    ...
    //mikeaz: inspired by OZ MerkleProof._processProof
    function processProof(bytes32[] calldata proof, bytes32 leaf) internal pure returns (bytes32) {
        bytes32 rootHash = leaf;
        for (uint256 i = 0; i < proof.length; i++) {
            rootHash = hashPair(rootHash, proof[i]);
        }
        return rootHash;        
    }
